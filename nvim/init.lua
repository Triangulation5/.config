...
vim.lsp.config("rust_analyzer", { capabilities = vim.tbl_deep_extend( "force", vim.lsp.protocol.make_client_capabilities(), { textDocument = { completion = { completionItem = { snippetSupport = true, resolveSupport = { properties = { "documentation", "detail", "additionalTextEdits", }, }, }, }, }, }), settings = { ["rust-analyzer"] = { cargo = { allFeatures = true, }, checkOnSave = true, procMacro = { enable = true, }, lens = { enable = true, } } } }); vim.lsp.config("vtsls", { root_dir = vim.fs.root(0, { "jsconfig.json", "package.json", "tsconfig.json" }) }); vim.lsp.config("tinymist", { settings = { formatterMode = "typstyle", exportPdf = "never" } }); vim.diagnostic.config({ virtual_text = { prefix = "■", spacing = 6, hl_mode = "combine", format = function(d) return string.format("%s [%s]", d.message, d.source or d.code or "") end }, signs = true, underline = true, update_in_insert = false, severity_sort = true, float = { border = "rounded", header = "Diagnostic(s):", source = "always", focusable = true } }); local typst_loaded = false; vim.api.nvim_create_autocmd("FileType", { pattern = "typst", callback = function() if typst_loaded then return end typst_loaded = true vim.schedule(function() vim.cmd.packadd("typst-preview.nvim") local ok, typst = pcall(require, "typst-preview") if not ok then vim.notify("typst-preview failed to load") return end typst.setup({}) vim.notify("typst-preview initialized") end) end, })
vim.lsp.enable({ "pyright", "ruff", "marksman", "vtsls", "tinymist", "rust_analyzer", "harper_ls" })
