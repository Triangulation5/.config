 ...
 * respawns through a cooldown timer if wl-paste dies. A failed `cliphist list`
 * (boot-time store lock) keeps the last good snapshot and quietly retries
 * once, and `loaded` stays false until the first success so the surface never
 * claims an empty history while that first load is in flight.
 *
