...
The shell is QML all the way down, with no web UI. Surfaces are lazy loaded and
killed after they sit idle, so the shell stays light on RAM and CPU. The static
theme is a port of vague.nvim (cool, dark, low contrast), with an option to
pull the palette from the current wallpaper instead.

It is built on [Ricelin](https://github.com/Gakuseei/Ricelin) by
[Gakuseei](https://github.com/Gakuseei), and not all of it is hand-written:
parts of the shell and the scripts were written with AI assistance, and
features take code or ideas from other Quickshell projects. See
[CREDITS.md](../CREDITS.md) for the base and the full list.
...
[flickowoa's dotfiles](https://github.com/flickowoa/dotfiles), and face unlock
uses [howdy](https://github.com/boltgolt/howdy). Parts of the shell and the
scripts were written with AI assistance. See [CREDITS.md](../CREDITS.md).
