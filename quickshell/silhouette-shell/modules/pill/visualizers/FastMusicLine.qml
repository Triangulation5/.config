...
Rectangle {
    ...
    Process {
        ...
        /**
         * Also gated on an actual client stream: the capture is what keeps the
         * sink awake, so it must not outlive the audio it renders (Cava's
         * `playbackWanted` owns that policy and the grace behind it). The string
         * is already warm by the first note either way - a player opens its
         * stream before its first sample.
         */
