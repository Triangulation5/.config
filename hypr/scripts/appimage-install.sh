...
apps_dir="$data_home/appimages"
desktop_dir="$data_home/applications"
icon_dir="$data_home/silhouette/appimages"
registry="$data_home/silhouette/appimages.json"
...
install_appimage() {
	...
	local df_out="$desktop_dir/silhouette-$slug.desktop"

	# An app installed before the entry prefix changed still has its old
	# `ricelin-<slug>.desktop` beside this one, and two entries for one app rank
	# twice in the launcher. Reinstalling drops the legacy file as it writes the
	# new one, which is also the moment the registry starts pointing at it.
	rm -f "$desktop_dir/ricelin-$slug.desktop"
	{
		echo "[Desktop Entry]"
		...
		echo "Terminal=false"
		echo "X-Silhouette-AppImage=true"
	} >"$df_out"
