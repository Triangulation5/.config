...
/**
 * Read/write helpers for the named `hl.layer_rule` calls in decoration.lua:
 * locate, add and remove a rule by name. The shared field helpers (getField /
 * setField / escapeRe) live in fields.js and are re-exported here so the QML
 * side keeps calling SetDeco.getField / setField / getBlockField /
 * setBlockField unchanged.
 */
...
function getBlockField(text, blockName, name) {
    ...
    if (!blk)
        return "";
    return Fields.getField(blk.body, name);
}
...
function setBlockField(text, blockName, name, valueLiteral) {
    ...
    var res = Fields.setField(blk.body, name, valueLiteral);
    if (!res.ok)
        return { text: text, ok: false };
...
function findNamedRule(text, ruleName) {
    ...
    var nameRe = new RegExp("name\\s*=\\s*\"" + Fields.escapeRe(ruleName) + "\"");
    var m;
