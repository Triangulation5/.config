...
def analyze(wallpaper):
    ...
    for line in out.splitlines():
        ...
        bucket = buckets.setdefault(
            (int(h * 360) // 30) % 12, {"wsat": 0.0, "best": None}
        )
        bucket["wsat"] += count * s
...
def render_fastfetch(pill):
    ...
    if not tmpl.is_file():
        print(
            "wallcolors: config.jsonc.in missing in ~/.config/fastfetch, skipping "
...
def main():
    ...
    light = mean_l >= 0.40
    surf_sat = (
        min(sat, 0.26) if light else min(max(sat, 0.30 if chromatic else 0.0), 0.45)
    )
    acc_sat = (
        (min(sat + 0.18, 0.85) if light else min(max(sat, 0.30) + 0.12, 0.82))
        if chromatic
        else 0.05
    )
    if light:
    ...
    pill = {
        name: tint(hue, surf_sat, base + step) for name, step in zip(SURF_NAMES, steps)
    }
    ...
    try:
        b = {
            k: v["dark"]["color"]
            for k, v in matugen(tint(hue, sat, 0.45) if chromatic else "#787878")[
                "base16"
            ].items()
        }
