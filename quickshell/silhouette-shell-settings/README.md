...
```bash
qs -c silhouette-shell-settings ipc call settings hide   # open / hide / toggle
```
...
The `.*` in the title is for the window's own name here, `Silhouette Settings
(standalone)`: the title is how a copy finds its *own* window in the compositor's
toplevel list, and Quickshell's toplevels carry no pid, so this copy and the shell's
would otherwise be indistinguishable. Hyprland anchors rule regexes — a plain
